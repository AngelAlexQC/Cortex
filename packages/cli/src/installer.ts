...
## Project Overview
This project uses Cortex for persistent AI memory across sessions.

## Coding Guidelines

### Memory Integration
- Query \`cortex_context\` before implementing features
...
`;
  ...
  const windsurfContent = `# Windsurf AI Rules - Cortex Memory Integration

> Generated by Cortex Memory System - ${date}

## Memory Protocol
Always start with: cortex_context("current task summary")
  ...
  const codyConfig = {
    ...
    settings: {
      'cody.chat.preInstruction':
        'This project uses Cortex for persistent AI memory. Start sessions with cortex_context(). Save decisions with cortex_add(). Follow: EXPLORE → PLAN → CODE → VERIFY → COMMIT workflow.',
    },
  };
